...
def compute_exp(row_mptcp, row_tcp, results, index, csv_filename, option):
    if option == 'aggreg':
        ...
        index = 0 if float(row_tcp['seq_acked'])/float(row_tcp['con_time']) >= float(row_tcp['seq_acked_2'])/float(row_tcp['con_time_2']) else 1
        mptcp_tot_thr = float(row_mptcp['seq_acked']) / float(row_mptcp['con_time']) / 1000000.0 * 8.0
...
def xlabel_name(val,option):
    ...
    if option == 'percent_path':
        name = 'Bytes transmitted on the first path rate'
    elif val == 'capacity': 
...
def per_sched_and_filesize_box(csv_mptcp_s, csv_tcp_s, min_diff, max_diff, nbr_set, val_diff_1, val_diff_2, plotfile, name, nbr_sched):
    ...
    for j in range(nbr_set):
        for i in range(len(csv_mptcp_s)):
            ...
            if (i+1) % nbr_sched == 0:
                #data.append([])
                annotation.append(tuple((l-1, m+"kB")))
                if i + 1 < len(csv_mptcp_s):
                    vlines.append(l-0.5)
    ...
    plt.xlabel('Scheduler used for different file sizes', size = 'xx-large')
    ...
    plt.axis([xmin,xmax,-1,2.2])
    for (x,text) in annotation:
        plt.annotate(s=text,xy=[x,2.1], weight='bold', size='xx-large')
    plt.xticks(x_numbers,x_names, size='xx-large')
    #plt.ylim([-1,1])
    #plt.show()
    plt.savefig(os.path.join(plotfile[0], name + '_new_5.eps'))
    plt.close('all')
...
def per_filesize_and_sched_box(csv_mptcp_s, csv_tcp_s, min_diff, max_diff, nbr_set, val_diff_1, val_diff_2, plotfile, name, nbr_sched):
    ...
    for i in range(len(csv_mptcp_s)/nbr_sched):
        ...
        if i+1 < len(csv_mptcp_s)/nbr_sched:
            annotation.append(tuple((float(l - 1 - 0.13*(len(m)-1)),str(m)+"kB")))
            #data.append([])
    ...
    [xmin,xmax,ymin,ymax] = plt.axis()
    plt.axis([xmin,xmax,-1,1.5])
    for (x,text) in annotation:
        plt.annotate(s=text,xy=[x,1.4], weight='bold', size='xx-large')
    plt.xticks(x_numbers,x_names, fontsize='x-large')
    #plt.ylim([-1,1])
    #plt.show()
    plt.savefig(os.path.join(plotfile[0], name + '_new_bis_bis.eps'))
    plt.close('all')
...
def exp_aggr_ben(csv_mptcp_s, csv_tcp_s, min_diff, max_diff, nbr_set, val_diff_1, val_diff_2, plotfile, name):
    ...
    for i in range(len(csv_mptcp_s)):
        ...
        #plt.ylim([-1,1])
        prename = csv_mptcp_s[i].split("/")[-1]
        plt.savefig(os.path.join(plotfile[0], prename[:-4] + "_" + name + '.eps'))
        plt.close('all')
...
def exp_first_path_per_size_per_sched(csv_mptcp_s, csv_tcp_s, min_diff, max_diff, nbr_set, val_diff_1, val_diff_2, plotfile, name, nbr_sched):
    ...
    for i in range(len(csv_mptcp_s)/nbr_sched):
        ...
        for j in range(nbr_set):
       	...
       	annotation.append(tuple((float(l + n + 0.18 - 0.12*(len(m)-1)),str(m)+"kB")))
       	if i+1 < (len(csv_mptcp_s)/nbr_sched):
            #data.append([])
            #n += 1
            vlines.append(float(l + nbr_sched*nbr_set + 0.5))
            l += n
    ...
    [xmin,xmax,ymin,ymax] = plt.axis()
    plt.axis([xmin,xmax,ymin,ymax+0.10])
    for (x,text) in annotation:
        plt.annotate(s=text,xy=[x,ymax], weight='bold', size = 'xx-large')
    
    plt.xticks(x_numbers  , x_names, size = 'x-large')
    #plt.ylim([-1,1])
    plt.savefig(os.path.join(plotfile[0], name + 'all.eps'))
    plt.close('all')
...
exp_first_path_per_size_per_sched(args.mptcp.split(","), args.tcp.split(","), int(args.min), int(args.max), 2, args.val_first, args.val_second, dir_plot, 'first_path', int(args.nbr_sched))
#exp_first_path_per_size_per_sched(args.mptcp.split(","), args.tcp.split(","), int(args.min), int(args.max), 2, args.val_first, args.val_second, dir_plot, 'first_path_theory', int(args.nbr_sched))
