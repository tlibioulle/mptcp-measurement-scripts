...
def template_csv(csv_file):
    ...
    csv_file_writer.writerow(['filesize', 'con_time', 'seq_acked', \
                            ...
                            'queue_1', 'losses_1','bytes_transmitted_1', \
                            'reinjection_of_pkt_caused_by_1', 'capacity_2', \
                            'delay_2','queue_2', 'losses_2', \
            ...
            for dirname in dirnames:
                ...
                if len(s) != 2:
                    ...
                    with open(os.path.join(name,filename_start+'_sf_'+str(con_id)+'.csv'),'rb') as analyzed_csv:
                        ...
                        for row in reader:
                            if j == 0:
                                to_write[8] = row[1+(1-con_id)] #bytes transmitted per sf 1
                            elif j == 1:
                                to_write[14] = row[1+(1-con_id)] #bytes transmitted per sf 2
                            j = j + 1
                    ...
                    with open(os.path.join(name,sender+'_stats_'+str(con_id)+'.csv'),'rb') as analyzed_csv:
                        ...
                        for row in reader:
                            if j == 2:
                                ...
                                to_write[9] = row[3+(1-con_id)] #reinjected caused by 1
                            elif j == 70:
                                to_write[15] = row[3+(1-con_id)] #reinjected caused by 2
                            j = j + 1
            ...
            for dirname in dirnames:
                ...
                if dirname in topos.keys():
                    ...
                    with open(os.path.join(name2,sender+'_stats_'+str(con_id)+'.csv'),'rb') as analyzed_csv:
                        ...
                        for row in reader:
                            if j == 2:
                                to_write[16] = row[3+(1-con_id)] # connection time
                            elif j == 3:
                                to_write[17] = row[3+(1-con_id)] # Nbr of seq acked
                            elif j == 7:
                                to_write[18] = row[3+(1-con_id)] #injected once
                            elif j == 72:
                                to_write[15] = row[3+(1-con_id)] #reinjected caused by 2
                            j = j + 1
...
def preprocess_exp_topo(csv_topos):
    ...
    with open(csv_topos, 'rb') as topos:
        ...
        for row in topos_reader:
            all_topos[str(row['Capacity1'])+'_'+str(row['Delay1'])+'_'+str(row['Queuing1'])+'_'+str(row['Losses1'])] \
                = str(row['Capacity2'])+'_'+str(row['Delay2'])+'_'+str(row['Queuing2'])+'_'+str(row['Losses2'])
